...
public class NumbersSampleFlow {
  ...
  public void run(Configuration configuration) {
    ...
    System.out.println("+----------------------------------------------------------------------------------------------------+");
    System.out.println("| Step 1: List the available regions that support the type 'LOCAL' and check is 'US' is part of them |");
    System.out.println("+----------------------------------------------------------------------------------------------------+");

    // Build the request data for listing the regions that support 'LOCAL' numbers
    var availableRegionsRequestData = AvailableRegionListAllRequestParameters.builder()
    ...
    // Even if not paginated, this API is following the SDK pattern for list and the server response is wrapped inside a ListResponse
    ...
    if (regions.isEmpty()) {
      LOGGER.severe("No regions are available for the number type " + availableRegionsRequestData.getTypes().orElseThrow().stream().map(NumberType::valueOf).collect(Collectors.joining(", ")));
      return;
    }

    // Check if 'US' is part of the regions that support the 'LOCAL' type
    System.out.println("List of available regions: " + regions.stream().map(Region::getRegionCode).collect(Collectors.joining(", ")));
    String regionCode = "";
    ...
    var availableNumbersRequestData = AvailableNumberListAllRequestParameters.builder()
        .setRegionCode(regionCode)
        .setType(NumberType.LOCAL)
        .setNumberPattern(NumberPattern.builder()
            .setSearchPattern(SearchPattern.START)
            .setPattern("+1781").build()) // Boston area code
        .build();
    ...
    var rentRequestData = AvailableNumberRentRequestParameters.builder()
        .build();
    ...
    System.out.println("The number " + rentedNumber.getPhoneNumber() + " has been rented. The next charge date is " + rentedNumber.getNextChargeDate());
    System.out.println("Let's use now the 'ActiveNumbers' API to see how to manage this number.\n");

    // NOTE: the 2 calls above can be merged in one with 'rentAny` operation. Feel free to uncomment the following code and try it out
...
//    }
//
//    System.out.println("The number " + rentedAnyNumber.getPhoneNumber() + " has been rented with the 'rentAny' operation. The next charge date is " + rentedAnyNumber.getNextChargeDate() + "\n");

    System.out.println("+----------------------------------------------------------------------------------------------+");
    System.out.println("| Step 4: Verify the number is part of our active numbers - Method 1: Give the number in input |");
    System.out.println("+----------------------------------------------------------------------------------------------+");
    ...
    System.out.println("SUCCESS: The number " + phoneNumber + " has been found as an active number. Cost is " + activeNumber.getMoney().getAmount() + " " + activeNumber.getMoney().getCurrencyCode() + "\n");

    System.out.println("+--------------------------------------------------------------------------------------------+");
    System.out.println("| Step 4: Check the number is part of our active numbers - Method 2: List all active numbers |");
    System.out.println("+--------------------------------------------------------------------------------------------+");

    var listActiveNumbersRequestData = ActiveNumberListRequestParameters.builder()
        .setRegionCode(regionCode)
        .setType(NumberType.LOCAL)
    ...
    // First possibility to check is the phone number is part of the active numbers: check the page content and if not present, check the next page until we find the phone number, or we reach the end of the list
    boolean found = false;
    // The 'list' operation supports pagination: the response content is wrapped inside the 'ListResponse' interface
    List<ActiveNumber> activeNumbersList = (List<ActiveNumber>) activeNumbersListResponse.getContent();
    ...
    for (int i = 0; i < numbersListSize; i++) {
      if (phoneNumber.equals(activeNumbersList.get(i).getPhoneNumber())) {
        ...
        // Check if we are at the end of the current page and if there are more pages to load from the server
        if (i == numbersListSize - 1 && activeNumbersListResponse.hasNextPage()) {
          // If yes, load more active numbers: 'nextPage()' will perform an HTTP call to fetch more active numbers
    ...
    // Second possibility to check if the phone  number is part of the active numbers: use the auto-iterator. Be careful, HTTP calls will be done under the hood without explicit mention in the source code
    activeNumbersListResponse = sinch.numbers().active().list(listActiveNumbersRequestData);
    var activeNumbersIterator = activeNumbersListResponse.autoPageIter();
    found = false;
    while (activeNumbersIterator.hasNext()) {
      var number = activeNumbersIterator.next(); // If we are at the end of the iterator content, the next() method will perform an HTTP call to load more elements in the iterator
    ...
    var updateActiveNumberRequestData = ActiveNumberUpdateRequestParameters.builder()
    ...
    System.out.println("SUCCESS: The number " + phoneNumber + " has been released; it will expire at " + releasedNumber.getExpireAt());
