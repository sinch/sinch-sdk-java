...
public class Utils {
  ...
  public static Logger initializeLogger(String className) {
    try (InputStream logConfigInputStream =
        NumbersSampleFlow.class.getClassLoader().getResourceAsStream("logging.properties")) {
  ...
  public static Properties loadProperties(Logger logger) {
    ...
    try (InputStream input =
        NumbersSampleFlow.class.getClassLoader().getResourceAsStream("config.properties")) {
      if (input != null) {
